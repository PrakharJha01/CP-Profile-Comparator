...
# Using BeautifulSoup to scrape Codeforces profile data
def get_profile(handle):
    ...
    # Avatar
    avatar = soup.find_all("div", class_="title-photo")
    for div in avatar:
        img = div.find("img")
        if img:
            avatar = img
    ...
    # Find Handle Rank/Tag
    tags=['gray','green','cyan','blue','violet','orange','red']
    for i in tags:
        tag2 = soup.find("span", class_=f"user-{i}")
        if tag2:
            m = i
            break
    # Tag
    tag = soup.find("span", class_=f"user-{m}")
    data["Tag"] = tag.text.strip() if tag else "N/A"

    # Rating
    rat = soup.find_all("span", class_=f"user-{m}")
    rating = rat[1].text.strip() if rat else "N/A"
    max_el = soup.find("span", class_="smaller")
    max_rat = max_el.find_all("span", class_=f"user-{m}") if max_el else None
    max_rating = max_rat[1].text.strip() if max_rat else "N/A"

    data["Rating"] = rating
    data["Max Rating"] = max_rating

    # Problem Solved
    problem_element = soup.find(class_="_UserActivityFrame_counterValue")
    data["Problems Solved"] = problem_element.get_text(strip=True) if problem_element else "N/A"
...
if compare_clicked:
    ...
    if p1 is None or p2 is None:
        st.error("Could not fetch one of the profiles.")
        st.stop()
